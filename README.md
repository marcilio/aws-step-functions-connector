...
This is a work in progress :) but is in pretty good shape at this point.
...
```bash
export templates_s3_bucket="[s3-bucket-that-will-contain-jinja2-templates]"
export lambda_package_s3_bucket="[s3-bucket-that-will-store-deployment-artifacts]"
```

* Copy the sample Jinja2 template to the S3 bucket:

```bash
aws s3 cp jinja/sample.jinja2 s3://[s3-bucket-that-will-contain-jinja2-templates]
```
